...
def _format_picks(picks: list) -> str:
    ...
    lines = ["<b>AI Tipster - Picks del dia</b>\n"]
    ...
    for tournament, t_picks in by_tournament.items():
        ...
        for pick in seen.values():
            ...
            lines.append(
                ...
                f"   EV: <b>{pick['ev']:+.1%}</b>  |  Stake: <b>{pick['stake_eur']} EUR</b>\n"
                f"   Modelo {pick['our_prob']:.0%}  vs  Mercado {pick['market_prob']:.0%}"
            )
        lines.append("")

    total_stake = sum(p["stake_eur"] for p in picks)
    lines.append(f"<b>Stake total: {total_stake:.2f} EUR</b>  ({len(picks)} pick(s))")
    return "\n".join(lines)
...
def send_roi_summary(rows: list) -> bool:
    if not rows:
        return _send("<b>ROI Summary</b> - Sin picks con resultado aun.")
    ...
    for r in rows:
        ...
        sign = "+" if roi > 0 else ""
        lines.append(
            f"<b>{r['tour']} {r['surface']}</b>\n"
            f"   {r['wins']}W/{r['losses']}L  |  {r['staked']} EUR apostado  |  "
            f"P&L: {r['profit']:+.2f} EUR  |  ROI: <b>{sign}{roi:.1f}%</b>"
        )
    return _send("\n".join(lines))


def send_unresolved_names_alert(names: list) -> bool:
    lines = [f"<b>AI Tipster - {len(names)} nombre(s) sin resolver</b>\n"]
    for r in names:
        lines.append(f"  - <code>{r['odds_name']}</code>  ({r['tour'] or '?'})  x{r['seen_count']}")
    lines.append("\nEjecuta el workflow <b>Player Name Aliases</b> en GitHub Actions para resolverlos.")
    return _send("\n".join(lines))
